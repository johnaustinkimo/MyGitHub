#!/bin/bash
#
# check_redfish_ssd_endurance_batch_v4.5.sh
#
# Purpose:
...
#
# v4.5 Fix:
#   1. Every host in servers.ini is guaranteed to appear in Server Summary.
#   2. Hosts with no populated SSD after filtering are shown as UNKNOWN / NO_DATA.
#   3. Hosts with API failure are shown as UNKNOWN, not silently missing.
#   4. Keeps iLO4 / iLO5 / iLO6 compatibility.
#   5. Keeps Empty Bay / Absent filtering.
#   6. Keeps SerialNumber de-duplication.
#
...
v4.5:
  Guarantee every servers.ini host appears in Server Summary.
EOF
}
...
check_one_server() {
  ...
  while read -r drive_uri; do
    ...
    if ! echo "$drive_json" | is_json_ok; then
      ...
      #######################################################################
      # SSD only. Keep UnknownMedia only when useful iLO4 legacy data exists.
      #######################################################################
...
<div class="footer">
Report generated by check_redfish_ssd_endurance_batch_v4.5.sh
</div>
