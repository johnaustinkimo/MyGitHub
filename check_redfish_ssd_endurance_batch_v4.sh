#!/bin/bash
#
# check_redfish_ssd_endurance_batch_v4.sh
#
# Purpose:
...
#
# iLO4/iLO5 SmartStorage commonly exposes:
#   SSDEnduranceUtilizationPercentage
#
# iLO6 / DMTF Drive model may expose life-left style fields depending on device,
# firmware, controller, and storage model. If life-left is found, convert:
#   used_percent = 100 - life_left_percent
#
# Output format:
...
extract_endurance_used() {
  ...
  # 1. Exact HPE legacy SmartStorage field, any depth.
  raw="$(echo "$json" | jq -r '
    first(
      paths(scalars) as $p
      | select(($p[-1] | tostring) == "SSDEnduranceUtilizationPercentage")
      | getpath($p)
    ) // empty
  ' 2>/dev/null)"
  ...
  # 2. Common used-percent style names, any depth.
  raw="$(echo "$json" | jq -r '
    first(
      paths(scalars) as $p
      | select(
          ($p[-1] | tostring | ascii_downcase)
          | test("^(percentagelifeused|percentused|percentageused|medialifeusedpercent|lifeusedpercent|wearpercent|wearlevelpercent|ssdwearoutpercent|ssdpercentused)$")
        )
      | getpath($p)
    ) // empty
  ' 2>/dev/null)"
  ...
  # 3. DMTF / vendor life-left style fields, any depth.
  #    Convert life-left percent to used percent.
  raw="$(echo "$json" | jq -r '
    first(
      paths(scalars) as $p
      | select(
          ($p[-1] | tostring | ascii_downcase)
          | test("(predictedmedialifeleftpercent|medialifeleftpercent|lifeleftpercent|ssdenduranceleftpercent|remaininglifepercent)")
        )
      | getpath($p)
    ) // empty
  ' 2>/dev/null)"
  ...
  # 4. No known endurance field found.
...
${found_drives}"
        ...
        # Drives as collection:
        #   "Drives": {"@odata.id": ".../Drives/"}
        drives_collection_uri="$(echo "$storage_member_json" | jq -r '.Drives."@odata.id" // empty')"
        ...
        if [[ -n "$drives_collection_uri" ]]; then
          ...
          if echo "$drives_collection_json" | is_json_ok; then
            found_drives="$(echo "$drives_collection_json" | jq -r '.Members[]?."@odata.id"')"
...
${found_drives}"
  ...
  if echo "$chassis_json" | is_json_ok; then
    chassis_uris="$(echo "$chassis_json" | jq -r '.Members[]?."@odata.id"')"
...
Compatible Paths: SmartStorage / Storage / Chassis Drives
</div>
...
<div class="footer">
Report generated by check_redfish_ssd_endurance_batch_v4.sh
</div>
