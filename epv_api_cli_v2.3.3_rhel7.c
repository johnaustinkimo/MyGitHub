...
/* defaults */
#define EPV_CLIENT_VERSION         "2.3.3"
...
#define DEFAULT_PRIORITY_PROBE_TIMEOUT 1
#define MAX_PRIORITY_PROBE_TIMEOUT    10
#define CACHE_NAMESPACE_HEX_LEN       16
#define CACHE_LOCK_TIMEOUT_MS         250U
...
typedef struct {
    ...
    int priority_probe_timeout_sec;
...
static void print_usage(RunMode mode)
{
    ...
    (void)fprintf(stderr, "  --priority-probe-timeout N Fast recovery HEALTH probe timeout seconds (default: %d, range: 1-%d)\n",
                  DEFAULT_PRIORITY_PROBE_TIMEOUT, MAX_PRIORITY_PROBE_TIMEOUT);
...
#ifndef _WIN32
/*
 * Cache isolation is derived from BOTH the executable identity and the selected
 * server-file path.  This prevents epv_api and epv_api_second (or one binary
 * invoked with different --server-file values) from sharing stale active-server
 * and token-origin state merely because they run under the same uid.
 */
static int cache_namespace_hex(const AppConfig *cfg, char out[CACHE_NAMESPACE_HEX_LEN + 1])
{
    char exe_path[PATH_MAX + 1];
    char server_path[PATH_MAX + 1];
    char seed[(PATH_MAX * 2) + 64];
    char digest[65];
    ssize_t nr;
    int n;
    ...
    nr = readlink("/proc/self/exe", exe_path, sizeof(exe_path) - 1U);
    if (nr <= 0) {
        (void)snprintf(exe_path, sizeof(exe_path), "unknown-exe");
    } else {
        exe_path[nr] = '\0';
    }

    if (realpath(cfg->server_file, server_path) == NULL) {
        if (copy_string_checked(server_path, sizeof(server_path), cfg->server_file) != 0) return -1;
    }

    n = snprintf(seed, sizeof(seed), "%s|%s|%d", exe_path, server_path, SERVER_PORT);
    if (n <= 0 || (size_t)n >= sizeof(seed) || token_sha256_hex(seed, digest) != 0) {
        secure_bzero(seed, sizeof(seed));
        secure_bzero(digest, sizeof(digest));
        return -1;
    }
    memcpy(out, digest, CACHE_NAMESPACE_HEX_LEN);
    out[CACHE_NAMESPACE_HEX_LEN] = '\0';
    secure_bzero(seed, sizeof(seed));
    secure_bzero(digest, sizeof(digest));
    return 0;
}

/* Cache is an optimization only: never let an advisory lock stall the CLI. */
static int flock_bounded(int fd, int operation, unsigned int timeout_ms, int debug, const char *label)
{
    ...
    for (;;) {
        ...
        if (errno != EWOULDBLOCK && errno != EAGAIN) {
            if (debug) perror(label != NULL ? label : "cache flock");
            return -1;
        }
        if (waited >= timeout_ms) {
            if (debug) {
                (void)fprintf(stderr, "[DEBUG] %s lock busy for >%u ms; bypassing cache\n",
                              label != NULL ? label : "cache", timeout_ms);
            }
            return -1;
        }
        msleep_unsigned(step_ms);
        waited += step_ms;
    }
}

static int token_origin_cache_path(const AppConfig *cfg, char *path, size_t path_size)
{
    const char *xdg = getenv("XDG_RUNTIME_DIR");
    struct stat st;
    char ns[CACHE_NAMESPACE_HEX_LEN + 1];
    int n;

    if (cache_namespace_hex(cfg, ns) != 0) return -1;
    if (xdg != NULL && *xdg != '\0' && stat(xdg, &st) == 0 && S_ISDIR(st.st_mode) &&
        st.st_uid == geteuid() && access(xdg, W_OK) == 0) {
        n = snprintf(path, path_size, "%s/epv_api_token_origin_%s.cache", xdg, ns);
    } else {
        n = snprintf(path, path_size, "/tmp/epv_api_token_origin_%lu_%s.cache",
                     (unsigned long)geteuid(), ns);
    }
...
static int token_origin_store(const char *token_response, const char *server, const AppConfig *cfg)
{
    char token[4096];
    char digest[65];
    ...
    if (token_sha256_hex(token, digest) != 0 || token_origin_cache_path(cfg, path, sizeof(path)) != 0) {
        secure_bzero(token, sizeof(token));
        return -1;
    }
    fd = open(path, O_WRONLY | O_CREAT | O_APPEND | O_CLOEXEC | O_NOFOLLOW, 0600);
    if (fd < 0) {
        if (cfg != NULL && cfg->debug) perror("token origin cache open");
        secure_bzero(token, sizeof(token));
        return -1;
    }
    ...
    (void)fchmod(fd, 0600);
    if (flock_bounded(fd, LOCK_EX, CACHE_LOCK_TIMEOUT_MS, (cfg != NULL) ? cfg->debug : 0, "token origin cache") != 0) {
        close(fd);
        secure_bzero(token, sizeof(token));
        return -1;
    }
    if (st.st_size > (off_t)(1024 * 1024)) {
        if (ftruncate(fd, 0) != 0 && cfg != NULL && cfg->debug) perror("token origin cache truncate");
    }
    ...
    (void)flock(fd, LOCK_UN);
    close(fd);
    if (cfg != NULL && cfg->debug) (void)fprintf(stderr, "[DEBUG] token origin cached server=%s digest=%.12s... path=%s\n", server, digest, path);
    secure_bzero(token, sizeof(token));
    secure_bzero(digest, sizeof(digest));
    return 0;
}

static int token_origin_lookup(const char *token_arg, char *server, size_t server_size, const AppConfig *cfg)
{
    char digest[65];
    char path[MAX_PATH_BUF];
    FILE *fp;
    ...
    if (token_sha256_hex(token_arg, digest) != 0 || token_origin_cache_path(cfg, path, sizeof(path)) != 0) return 0;
    fp = fopen(path, "r");
    if (fp == NULL) {
        secure_bzero(digest, sizeof(digest));
        return 0;
    }
    if (flock_bounded(fileno(fp), LOCK_SH, CACHE_LOCK_TIMEOUT_MS, (cfg != NULL) ? cfg->debug : 0, "token origin cache") != 0) {
        (void)fclose(fp);
        secure_bzero(digest, sizeof(digest));
        return 0;
    }
    ...
    if (cfg != NULL && cfg->debug) (void)fprintf(stderr, "[DEBUG] token origin affinity server=%s path=%s\n", server, path);
    return 1;
}
#else
static int token_origin_store(const char *token_response, const char *server, const AppConfig *cfg)
{
    (void)token_response; (void)server; (void)cfg; return 0;
}
static int token_origin_lookup(const char *token_arg, char *server, size_t server_size, const AppConfig *cfg)
{
    (void)token_arg; (void)server; (void)server_size; (void)cfg; return 0;
}
#endif
...
static void init_config(AppConfig *cfg)
{
    ...
    cfg->priority_probe_timeout_sec = DEFAULT_PRIORITY_PROBE_TIMEOUT;
...
static int wait_socket_ready(socket_handle_t sock, int want_read, int want_write,
                             unsigned long long deadline_ms)
{
    for (;;) {
        ...
        tv.tv_sec = (long)(remain / 1000ULL);
        tv.tv_usec = (long)((remain % 1000ULL) * 1000ULL);

        FD_ZERO(&rfds);
        FD_ZERO(&wfds);
        if (want_read) FD_SET(sock, &rfds);
        if (want_write) FD_SET(sock, &wfds);

        rc = select((int)(sock + 1), want_read ? &rfds : NULL,
...
static int ssl_connect_with_timeout(SSL *ssl, socket_handle_t sock,
                                    int timeout_sec, int debug)
{
    unsigned long long now;
    unsigned long long deadline;
    int rc = -1;

    if (ssl == NULL || timeout_sec <= 0) return -1;
    now = monotonic_millis();
    if (now == 0ULL) return -1;
    deadline = now + ((unsigned long long)timeout_sec * 1000ULL);
    ...
    for (;;) {
        ...
        }
        {
            ...
            if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
                int wr = wait_socket_ready(sock,
                                           err == SSL_ERROR_WANT_READ,
                                           err == SSL_ERROR_WANT_WRITE,
                                           deadline);
                if (wr > 0) continue;
                if (debug) {
                    (void)fprintf(stderr, "[DEBUG] TLS handshake timeout after %d sec\n", timeout_sec);
                }
                break;
            }
            if (debug) {
                (void)fprintf(stderr, "[DEBUG] SSL_connect failed: err=%d\n", err);
                ERR_print_errors_fp(stderr);
            }
            break;
        }
    }
    if (set_nonblocking(sock, 0) != 0 && rc == 0) rc = -1;
    return rc;
}

static int ssl_send_all(SSL *ssl, const char *buf, size_t len, int debug, int timeout_sec)
{
    ...
    if (ssl == NULL || buf == NULL || timeout_sec <= 0) return -1;
    sock = (socket_handle_t)SSL_get_fd(ssl);
    now = monotonic_millis();
    if (now == 0ULL) return -1;
    deadline = now + ((unsigned long long)timeout_sec * 1000ULL);
    ...
    while (sent < len) {
        ...
        }
        {
            ...
            if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
                if (wait_socket_ready(sock,
                                      err == SSL_ERROR_WANT_READ,
                                      err == SSL_ERROR_WANT_WRITE,
                                      deadline) > 0) {
                    continue;
                }
                if (debug) (void)fprintf(stderr, "[DEBUG] SSL_write timeout after %d sec\n", timeout_sec);
                goto done;
            }
            if (debug) {
...
static int ssl_read_response_line(SSL *ssl, char *response, size_t response_size,
                                  int debug, int timeout_sec)
{
    socket_handle_t sock;
    unsigned long long now;
    unsigned long long deadline;
    ...
    if ((ssl == NULL) || (response == NULL) || (response_size < 2U) || timeout_sec <= 0) return -1;
    response[0] = '\0';
    sock = (socket_handle_t)SSL_get_fd(ssl);
    now = monotonic_millis();
    if (now == 0ULL) return -1;
    ...
    while (total + 1U < response_size) {
...

        {
            ...
            if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
                ...
                if (debug) (void)fprintf(stderr, "[DEBUG] SSL_read timeout after %d sec\n", timeout_sec);
                goto done;
            }
                            ...
                            const AppConfig *cfg)
{
    ...
    if (ssl_connect_with_timeout(ssl, sock, cfg->io_timeout_sec, cfg->debug) != 0) {
        goto cleanup;
    }
    ...
    if (ssl_send_all(ssl, message, strlen(message), cfg->debug, cfg->io_timeout_sec) != 0) goto cleanup;
    if (ssl_read_response_line(ssl, response, response_size, cfg->debug, cfg->io_timeout_sec) != 0) goto cleanup;
                                 ...
                                 const AppConfig *cfg)
{
    ...
    /*
     * A populated external server file is authoritative.  Do not escape into
     * the compiled-in cluster, otherwise two separately configured clients can
     * unexpectedly cross-connect.  Built-ins are emergency fallback only when
     ...
     */
    {
        ...
        if (ext_count == 0) {
            const char *const *bp;
            if (cfg->debug) {
                (void)fprintf(stderr, "[DEBUG] server file %s empty/unavailable; token-owner probe may use built-in fallback\n",
                              cfg->server_file);
            }
...
static int response_is_active_health(const char *response)
{
    if (response == NULL) return 0;
    if (strncmp(response, "ok ", 3U) != 0) return 0;
    return (strstr(response, " role=ACTIVE") != NULL) ? 1 : 0;
}

static int fast_priority_health_probe(const char *server, const AppConfig *cfg)
{
    AppConfig probe_cfg;
    char response[MAX_RESPONSE_LEN];

    if (server == NULL || cfg == NULL) return 0;
    probe_cfg = *cfg;
    probe_cfg.connect_timeout_sec = cfg->priority_probe_timeout_sec;
    probe_cfg.io_timeout_sec = cfg->priority_probe_timeout_sec;
    probe_cfg.retries_per_server = 1;
    memset(response, 0, sizeof(response));

    if (cfg->debug) {
        (void)fprintf(stderr,
                      "[DEBUG] Fast priority HEALTH probe server=%s timeout=%d sec retries=1\n",
                      server, cfg->priority_probe_timeout_sec);
    }
    if (try_server_with_retries(server, "HEALTH\n", response, sizeof(response), &probe_cfg) != 0) {
        if (cfg->debug) (void)fprintf(stderr, "[DEBUG] Fast priority probe failed server=%s\n", server);
        return 0;
    }
    if (response_is_active_health(response) == 0) {
        if (cfg->debug) {
            (void)fprintf(stderr, "[DEBUG] Fast priority probe server=%s is not ACTIVE; response=%s",
                          server, response[0] ? response : "(empty)\n");
        }
        return 0;
    }
    if (cfg->debug) (void)fprintf(stderr, "[DEBUG] Fast priority probe ACTIVE server=%s\n", server);
    return 1;
}
...
static int build_ordered_servers(const AppConfig *cfg, char servers[][MAX_SERVER_LEN], int max_servers)
{
    ...
    if (ext_count > 0) {
        if (cfg->debug) {
            (void)fprintf(stderr, "[DEBUG] server file is authoritative: %s entries=%d; built-in fallback disabled for this run\n",
                          cfg->server_file, ext_count);
        }
        return count;
    }
    if (cfg->debug) {
        (void)fprintf(stderr, "[DEBUG] server file %s empty/unavailable; using compiled-in fallback servers\n",
                      cfg->server_file);
    }
...
#ifndef _WIN32
static int priority_cache_path(const AppConfig *cfg, char *path, size_t path_size)
{
    const char *xdg = getenv("XDG_RUNTIME_DIR");
    struct stat st;
    char ns[CACHE_NAMESPACE_HEX_LEN + 1];
    ...
    if (xdg != NULL && *xdg != '\0' && stat(xdg, &st) == 0 && S_ISDIR(st.st_mode) &&
        st.st_uid == geteuid() && access(xdg, W_OK) == 0) {
        n = snprintf(path, path_size, "%s/epv_api_active_server_%s.cache", xdg, ns);
    } else {
        n = snprintf(path, path_size, "/tmp/epv_api_active_server_%lu_%s.cache",
                     (unsigned long)geteuid(), ns);
    }
    return (n > 0 && (size_t)n < path_size) ? 0 : -1;
}
...
typedef enum {
    PRIORITY_CACHE_NONE = 0,
    PRIORITY_CACHE_USE = 1,
    PRIORITY_CACHE_RECHECK_DUE = 2
} PriorityCachePlan;

/*
 * Return PRIORITY_CACHE_USE when a recently-working node should be used first.
 * Return PRIORITY_CACHE_RECHECK_DUE when a lower-priority cached node remains
 * usable but higher-priority nodes should be checked with the fast HEALTH probe.
 ...
 */
static int priority_cache_plan(const AppConfig *cfg, char servers[][MAX_SERVER_LEN], int count,
                               ...
                               int debug)
{
    ...
    if (recheck_sec <= 0 || count <= 0 || priority_cache_path(cfg, path, sizeof(path)) != 0) return 0;
    fd = priority_cache_open_secure(path, debug);
    if (fd < 0) return 0;
    if (flock_bounded(fd, LOCK_EX, CACHE_LOCK_TIMEOUT_MS, debug, "priority cache") != 0) { close(fd); return 0; }
    ...
    if (rank == 0 || now < (time_t)next_recheck) {
        ...
        if (ok && debug) fprintf(stderr, "[DEBUG] priority cache selects %s rank=%d next_recheck=%ld path=%s\n", server, rank, next_recheck, path);
        return ok ? PRIORITY_CACHE_USE : PRIORITY_CACHE_NONE;
    }
    /* Lease the recovery probe now. Other concurrent clients keep using the cached node. */
    (void)priority_cache_write_locked(fd, now + (time_t)recheck_sec, server);
    {
        int ok = (copy_string_checked(cached, cached_size, server) == 0) ? 1 : 0;
        (void)flock(fd, LOCK_UN); close(fd);
        if (!ok) return PRIORITY_CACHE_NONE;
    }
    if (debug) fprintf(stderr, "[DEBUG] priority recovery recheck due; fast-probing above cached server %s\n", server);
    return PRIORITY_CACHE_RECHECK_DUE;
}

static void priority_cache_store(const AppConfig *cfg, const char *server, int recheck_sec, int debug)
{
    char path[MAX_PATH_BUF];
    int fd;
    time_t now;
    if (server == NULL || *server == '\0' || recheck_sec <= 0 || priority_cache_path(cfg, path, sizeof(path)) != 0) return;
    fd = priority_cache_open_secure(path, debug);
    if (fd < 0) return;
    if (flock_bounded(fd, LOCK_EX, CACHE_LOCK_TIMEOUT_MS, debug, "priority cache") != 0) { close(fd); return; }
    now = time(NULL);
    (void)priority_cache_write_locked(fd, now + (time_t)recheck_sec, server);
    (void)flock(fd, LOCK_UN);
    close(fd);
    if (debug) fprintf(stderr, "[DEBUG] active server cached=%s recheck_in=%d sec path=%s\n", server, recheck_sec, path);
}
#else
static int priority_cache_plan(const AppConfig *cfg, char servers[][MAX_SERVER_LEN], int count,
                               int recheck_sec, char *cached, size_t cached_size,
                               int debug)
{
    (void)cfg; (void)servers; (void)count; (void)recheck_sec; (void)cached; (void)cached_size; (void)debug;
    return 0;
}
static void priority_cache_store(const AppConfig *cfg, const char *server, int recheck_sec, int debug)
{
    (void)cfg; (void)server; (void)recheck_sec; (void)debug;
}
#endif
...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; ) {
        if (strcmp(argv[i], "--verify") == 0) {
            ...
            for (j = i; j < argc - 2; ++j) argv[j] = argv[j + 2];
            argc -= 2;
        } else if (strcmp(argv[i], "--priority-probe-timeout") == 0) {
            if ((i + 1 >= argc) ||
                (parse_positive_int(argv[i + 1], 1, MAX_PRIORITY_PROBE_TIMEOUT,
                                    &cfg.priority_probe_timeout_sec) != 0)) {
                print_usage(MODE_UNKNOWN);
                platform_cleanup();
                return (int)EXIT_USAGE;
            }
    ...
    if ((mode == MODE_DB) && (cfg.token_affinity != TOKEN_AFFINITY_OFF)) {
        ...
        have_origin = token_origin_lookup(argv[1], pinned_server, sizeof(pinned_server), &cfg);
        if (!have_origin) {
            ...
            if (have_origin) {
                (void)token_origin_store(argv[1], pinned_server, &cfg);
    ...
    if (success == 0) {
        ...
        cache_plan = (mode == MODE_HEALTH) ? PRIORITY_CACHE_NONE :
                     priority_cache_plan(&cfg, ordered, ordered_count, cfg.priority_recheck_sec,
                                         cached_server, sizeof(cached_server), cfg.debug);

        /*
         * Recovery rechecks must never turn a healthy cached ACTIVE node into a
         * long user-visible stall. Probe only nodes above the cached rank with a
         * non-destructive HEALTH request, one attempt, and a short hard TLS/I/O
         * deadline. Failed fast probes are marked tried for this invocation so
         * the normal fallback loop does not immediately repeat them with full
         * timeouts.
         */
        if (cache_plan == PRIORITY_CACHE_RECHECK_DUE) {
            int cached_rank = server_rank(ordered, ordered_count, cached_server);
            if (cached_rank > 0) {
                for (i = 0; i < cached_rank && success == 0; ++i) {
                    if (server_already_tried(tried, tried_count, ordered[i]) != 0) continue;
                    if (fast_priority_health_probe(ordered[i], &cfg) != 0) {
                        memset(response, 0, sizeof(response));
                        mark_server_tried(tried, &tried_count, ordered[i]);
                        if (cfg.debug) {
                            (void)fprintf(stderr, "[DEBUG] Higher-priority ACTIVE node recovered; sending request to %s\n", ordered[i]);
                        }
                        if (try_server_with_retries(ordered[i], message, response, sizeof(response), &cfg) == 0 &&
                            (mode == MODE_HEALTH || response_is_nonactive(response) == 0)) {
                            success = 1;
            ...
            if (success == 0 && cfg.debug) {
                (void)fprintf(stderr, "[DEBUG] No higher-priority ACTIVE node passed fast probe; using cached server %s now\n", cached_server);
            }
        }

        if (success == 0 && cache_plan != PRIORITY_CACHE_NONE &&
            server_already_tried(tried, tried_count, cached_server) == 0) {
            memset(response, 0, sizeof(response));
    ...
    exit_code = EXIT_OK;
    if (mode != MODE_HEALTH) {
        priority_cache_store(&cfg, chosen_server, cfg.priority_recheck_sec, cfg.debug);
    }
    if ((mode == MODE_AD || mode == MODE_APP_MD5 || mode == MODE_APP_SHA256 || mode == MODE_APP_CHECKSUM) &&
        cfg.token_affinity != TOKEN_AFFINITY_OFF) {
        (void)token_origin_store(response, chosen_server, &cfg);
    }
