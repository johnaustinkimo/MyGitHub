...
# v1.3.4 inner original-file logging defaults
...
if bool_yes "$POST_VERIFY"; then
    ...
    POST_VERIFY_LOG_INNER_FILES_FORMAT="${POST_VERIFY_LOG_INNER_FILES_FORMAT,,}"
    case "$POST_VERIFY_LOG_INNER_FILES_FORMAT" in
        names|verbose) ;;
        *) die "invalid POST_VERIFY_LOG_INNER_FILES_FORMAT=$POST_VERIFY_LOG_INNER_FILES_FORMAT (expected: names|verbose)" ;;
    esac

    [[ "$POST_VERIFY_LOG_INNER_FILES_MAX" =~ ^[0-9]+$ ]] && (( POST_VERIFY_LOG_INNER_FILES_MAX > 0 )) || \
        die "invalid positive integer setting: POST_VERIFY_LOG_INNER_FILES_MAX=$POST_VERIFY_LOG_INNER_FILES_MAX"
...
select_latest_file()
{
    local archive key best_key="" best_archive=""
    ...
    while IFS= read -r archive; do
        ...
        if [[ -z "$best_key" || "$key" > "$best_key" || ( "$key" == "$best_key" && "$archive" > "$best_archive" ) ]]; then
            best_key="$key"
    ...
    log INFO "latest cloud archive: $LATEST_FILE date_key=$best_key"
    return 0
}
...
log_inner_original_files()
{
    local member="$1" names_file="$2" verbose_file="${3:-}"
    local source_file effective_format total shown omitted line

    bool_yes "$POST_VERIFY_LOG_INNER_FILES" || return 0

    if [[ "$POST_VERIFY_LOG_INNER_FILES_FORMAT" == "verbose" && -s "$verbose_file" ]]; then
        source_file="$verbose_file"
        effective_format="verbose"
    else
        source_file="$names_file"
        effective_format="names"
    fi

    [[ -s "$source_file" ]] || {
        log WARNING "inner original-file log skipped: manifest missing/empty member=$member"
        return 0
    }

    total="$(wc -l < "$source_file" | awk '{print $1}')"
    [[ "$total" =~ ^[0-9]+$ ]] || total=0

    shown="$total"
    if (( shown > POST_VERIFY_LOG_INNER_FILES_MAX )); then
        shown="$POST_VERIFY_LOG_INNER_FILES_MAX"
    fi

    log INFO "inner original-file list started member=$member format=$effective_format showing=${shown}/${total}"

    while IFS= read -r line; do
        printf '%s [INFO] inner original file member=%s %s\n' \
            "$(date '+%Y-%m-%d %H:%M:%S')" "$member" "$line" | tee -a "$RUN_LOG"
    done < <(head -n "$POST_VERIFY_LOG_INNER_FILES_MAX" "$source_file")

    omitted=$(( total - shown ))
    if (( omitted > 0 )); then
        log INFO "inner original-file list truncated member=$member shown=$shown total=$total omitted=$omitted limit=$POST_VERIFY_LOG_INNER_FILES_MAX"
    fi

    log INFO "inner original-file list completed member=$member shown=$shown total=$total"
    return 0
}
...
safe_inner_tar_manifest()
{
    ...
    log_inner_original_files "$member" "$names_file" "$verbose_file"
    return 0
}
...
verify_one_signed_member()
{
    ...
    if ! bool_yes "$POST_VERIFY_EXTRACT_INNER"; then
        log_inner_original_files "$member" "$inner_list" ""
    fi
...
show_status()
{
    local marker archive path marker_size actual_size status verify_state verify_mode verify_total verify_verified verify_failed verify_extract regular_files readable_files checkpoint checkpoint_count checkpoint_total all_checkpoint_display
    local found=0
    ...
    while IFS= read -r marker; do
        ...
        checkpoint_count=0
        checkpoint_total=""
        all_checkpoint_display="none"
        if [[ -f "$checkpoint" ]]; then
            checkpoint_count="$(checkpoint_completed_count "$checkpoint")"
            checkpoint_total="$(read_marker_value "$checkpoint" total)"
            if [[ "$checkpoint_total" =~ ^[0-9]+$ ]] && (( checkpoint_total > 0 )); then
                all_checkpoint_display="${checkpoint_count}/${checkpoint_total}"
            else
                all_checkpoint_display="${checkpoint_count}/?"
            fi
        fi
        ...
        printf '  %-20s status=%-14s marker_size=%s actual_size=%s verify=%s mode=%s verified=%s/%s failed=%s extract=%s files=%s/%s all_checkpoint=%s\n' \
            "$archive" \
            "$status" \
            "${marker_size:-unknown}" \
            "${actual_size:-0}" \
...
run performs:
  ...
  -> large-file progress/timeout -> all-mode checkpoint/resume
  -> inner original-file logging -> completion marker
