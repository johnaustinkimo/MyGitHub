...
#  10. v1.6 adds mbuffer acceleration for Tape -> NFS restore:
#      Tape -> mbuffer (6G/1M default) -> tar -> NFS, with independent mbuffer
#      and tar return-code validation plus a --no-mbuffer compatibility path.
#
# Security note:
...
# Tape -> NFS restore buffering. Enabled by default to decouple sequential tape
# reads from NFS/tar extraction latency and reduce tape stop/start cycles.
USE_MBUFFER="${TS4500_USE_MBUFFER:-1}"
MBUFFER="${TS4500_MBUFFER:-mbuffer}"
BUFFER_MEM="${TS4500_BUFFER_MEM:-6G}"
BLOCK_SIZE="${TS4500_BLOCK_SIZE:-1M}"
...
usage() {
  cat <<'USAGE'
Usage:
  ts4500_ops_v1.6.sh [GLOBAL OPTIONS] COMMAND [COMMAND ARGS]
...
Global options:
  ...
  --mbuffer-bin PATH        mbuffer executable for Tape -> NFS restore
                           (default: mbuffer)
  --buffer-mem SIZE         mbuffer memory for restore (default: 6G)
  --block-size SIZE         mbuffer I/O block size for restore (default: 1M)
  --no-mbuffer              Disable buffering; use direct tar from tape
  --mbuffer                 Explicitly enable restore buffering
...
Commands:
  ...
  tape-read [VOLSER] [--to DIR] [--allow-existing] [--no-rewind] [--preserve-owner]
            [--mbuffer|--no-mbuffer] [--mbuffer-bin PATH]
            [--buffer-mem SIZE] [--block-size SIZE]
      Extract a tar archive from the loaded tape to NFS. By default the data
      path is Tape -> mbuffer -> tar -> NFS (6G buffer / 1M I/O block). If VOLSER is omitted,
  ...
  recover [VOLSER] [--to DIR] [--allow-existing] [--preserve-owner]
      ...
      Restore buffering defaults to mbuffer with 6G memory / 1M blocks.
      --no-mbuffer reverts to the v1.5 direct tape-to-tar restore path.
...
Examples:
  ./ts4500_ops_v1.6.sh drive-check
  ./ts4500_ops_v1.6.sh candidates
  VOL=$(./ts4500_ops_v1.6.sh select)
  ./ts4500_ops_v1.6.sh load "$VOL"
  ./ts4500_ops_v1.6.sh verify-loaded "$VOL"
  ./ts4500_ops_v1.6.sh tape-status
  ./ts4500_ops_v1.6.sh tape-probe
  ./ts4500_ops_v1.6.sh tape-probe "$VOL" --entries 10
  ./ts4500_ops_v1.6.sh tape-read          # mbuffer 6G/1M + NFS-safe ownership
  ./ts4500_ops_v1.6.sh tape-read --buffer-mem 6G --block-size 1M
  ./ts4500_ops_v1.6.sh tape-read --no-mbuffer # v1.5 direct restore behavior
  ./ts4500_ops_v1.6.sh recover --after unload
  ./ts4500_ops_v1.6.sh recover "$VOL" --after export --yes
  ./ts4500_ops_v1.6.sh unload             # auto-detect VOLSER and remember returned Slot
  ./ts4500_ops_v1.6.sh last-unloaded
  ./ts4500_ops_v1.6.sh export --yes       # export remembered VOLSER from remembered Slot
  ./ts4500_ops_v1.6.sh export "$VOL" --yes
  ./ts4500_ops_v1.6.sh unload-export --yes # unload + remember + export same tape
USAGE
}
...
validate_buffer_setting() {
  local name=$1 value=$2
  [[ "$value" =~ ^[1-9][0-9]*([kKmMgGtT])?$ ]] || \
    fail "$RC_USAGE" "$name must be a positive size such as 256M, 1G, or 6G: $value"
}

ensure_mbuffer() {
  (( USE_MBUFFER )) || return 0
  if [[ "$MBUFFER" == */* ]]; then
    [[ -x "$MBUFFER" ]] || fail "$RC_DEPENDENCY" \
      "mbuffer executable not found or not executable: $MBUFFER"
  else
    command -v "$MBUFFER" >/dev/null 2>&1 || fail "$RC_DEPENDENCY" \
      "mbuffer is enabled but command was not found: $MBUFFER (use --no-mbuffer for direct restore)"
  fi
}
...
validate_settings() {
  ...
  case "$USE_MBUFFER" in
    0|1) ;;
    *) fail "$RC_USAGE" "TS4500_USE_MBUFFER/USE_MBUFFER must be 0 or 1" ;;
  esac
  validate_buffer_setting buffer-mem "$BUFFER_MEM"
  validate_buffer_setting block-size "$BLOCK_SIZE"
...
cmd_tape_read() {
  ...
  local vol log_file tar_rc=0 mbuffer_rc=0 tee_rc=0 file_count bytes
  local start_epoch end_epoch elapsed restore_mode="DIRECT"
  local -a pipe_rc tar_extract_args

  # Command-specific arguments may appear in any order. Performance options are
  # accepted here as well as globally so existing operational syntax stays easy.
  while (( $# )); do
    case "$1" in
      --to)
      ...
      --no-mbuffer)
        ...
        ;;
      --mbuffer-bin)
        [[ $# -ge 2 ]] || fail "$RC_USAGE" "tape-read --mbuffer-bin requires a path"
        MBUFFER=$2
        shift 2
        ;;
      --buffer-mem)
        [[ $# -ge 2 ]] || fail "$RC_USAGE" "tape-read --buffer-mem requires a size"
        BUFFER_MEM=$2
        shift 2
        ;;
      --block-size)
        [[ $# -ge 2 ]] || fail "$RC_USAGE" "tape-read --block-size requires a size"
        BLOCK_SIZE=$2
        shift 2
        ;;
      --)
  ...
  if (( USE_MBUFFER )); then
    restore_mode="MBUFFER"
    info "Restore mode: Tape -> mbuffer -> tar -> NFS"
    info "mbuffer: binary=$MBUFFER memory=$BUFFER_MEM block_size=$BLOCK_SIZE"
  else
    restore_mode="DIRECT"
    info "Restore mode: direct tar from tape (--no-mbuffer)"
  fi
  ...
  if (( DRY_RUN )); then
    info "DRY-RUN: would $([[ $no_rewind -eq 1 ]] && printf 'not rewind; ' || printf 'rewind; ')extract tar archive from '$TAPE_DEVICE' to '$dest' mode=$restore_mode owner_policy=$([[ $preserve_owner -eq 1 ]] && printf 'preserve' || printf 'nfs-safe-no-same-owner') buffer_mem=$BUFFER_MEM block_size=$BLOCK_SIZE"
    printf '%s\n' "$dest"
    return 0
  fi
  ...
  info "Starting tar extraction: device=$TAPE_DEVICE volume=$vol destination=$dest mode=$restore_mode"
  info "Restore log: $log_file"

  if (( USE_MBUFFER )); then
    # mbuffer owns the tape-device read; tar consumes a clean byte stream on
    # stdin. Never merge mbuffer stderr into stdout/archive bytes.
    if (( preserve_owner )); then
      tar_extract_args=(-xvf - -C "$dest")
      info "Tar owner policy: preserve archived UID/GID (--preserve-owner requested)"
    else
      tar_extract_args=(--no-same-owner -xvf - -C "$dest")
      info "Tar owner policy: NFS-safe --no-same-owner (ownership determined by NFS/server)"
    fi
  else
    # v1.5 compatibility path: tar opens the tape device directly.
    if (( preserve_owner )); then
  ...
  {
    printf 'TS4500 tape restore\n'
    printf 'timestamp=%s\n' "$(ts)"
    printf 'volume=%s\n' "$vol"
    printf 'device=%s\n' "$TAPE_DEVICE"
    printf 'destination=%s\n' "$dest"
    printf 'restore_mode=%s\n' "$restore_mode"
    printf 'use_mbuffer=%s\n' "$USE_MBUFFER"
    printf 'mbuffer=%s\n' "$MBUFFER"
    printf 'buffer_mem=%s\n' "$BUFFER_MEM"
    printf 'block_size=%s\n' "$BLOCK_SIZE"
    printf '%s\n' '--- restore output ---'
  ...
  set +e
  if (( USE_MBUFFER )); then
    "$MBUFFER" -m "$BUFFER_MEM" -s "$BLOCK_SIZE" -i "$TAPE_DEVICE" \
      2> >(tee -a "$log_file" >&2) \
      | tar "${tar_extract_args[@]}" 2>&1 \
      | tee -a "$log_file"
    pipe_rc=("${PIPESTATUS[@]}")
    mbuffer_rc=${pipe_rc[0]:-99}
    tar_rc=${pipe_rc[1]:-99}
    tee_rc=${pipe_rc[2]:-99}
  else
  ...
  # tar is authoritative for archive/extraction correctness. If tar failed,
  # report RC_TAR first even if upstream mbuffer also sees SIGPIPE/broken pipe.
  if (( tar_rc != 0 )); then
    err "tar extraction failed: mode=$restore_mode tar_rc=$tar_rc mbuffer_rc=$mbuffer_rc volume=$vol device=$TAPE_DEVICE destination=$dest log=$log_file"
    exit "$RC_TAR"
  fi
  if (( mbuffer_rc != 0 )); then
    err "mbuffer tape read failed: rc=$mbuffer_rc volume=$vol device=$TAPE_DEVICE destination=$dest log=$log_file"
    exit "$RC_MBUFFER"
  fi
  if (( tee_rc != 0 )); then
    warn "Restore data completed but tee/log output returned rc=$tee_rc; review log filesystem: $log_file"
  fi
  ...
  info "Tape read completed successfully: volume=$vol mode=$restore_mode files=$file_count bytes=$bytes elapsed=${elapsed}s destination=$dest"
...
cmd_recover() {
  ...
  while (( $# )); do
    case "$1" in
      ...
      --no-mbuffer)
        ...
        ;;
      --mbuffer-bin)
        [[ $# -ge 2 ]] || fail "$RC_USAGE" "recover --mbuffer-bin requires a path"
        MBUFFER=$2
        shift 2
        ;;
      --buffer-mem)
        [[ $# -ge 2 ]] || fail "$RC_USAGE" "recover --buffer-mem requires a size"
        BUFFER_MEM=$2
        shift 2
        ;;
      --block-size)
        [[ $# -ge 2 ]] || fail "$RC_USAGE" "recover --block-size requires a size"
        BLOCK_SIZE=$2
        shift 2
        ;;
      --probe-entries)
  ...
  printf 'Read Mode    : %s\n' "$([[ $USE_MBUFFER -eq 1 ]] && printf 'MBUFFER' || printf 'DIRECT')" >&2
  if (( USE_MBUFFER )); then
    printf 'Buffer Mem   : %s\n' "$BUFFER_MEM" >&2
    printf 'Block Size   : %s\n' "$BLOCK_SIZE" >&2
  fi
...
while (( $# )); do
  case "$1" in
    ...
    --mbuffer-bin)     [[ $# -ge 2 ]] || fail "$RC_USAGE" "--mbuffer-bin requires a value"; MBUFFER=$2; shift 2 ;;
    --buffer-mem)      [[ $# -ge 2 ]] || fail "$RC_USAGE" "--buffer-mem requires a value"; BUFFER_MEM=$2; shift 2 ;;
    --block-size)      [[ $# -ge 2 ]] || fail "$RC_USAGE" "--block-size requires a value"; BLOCK_SIZE=$2; shift 2 ;;
    --no-mbuffer)      USE_MBUFFER=0; shift ;;
    --mbuffer)         USE_MBUFFER=1; shift ;;
