...
#define EPV_VERSION "2.3.2-snapshot"
...
static void process_request(worker_ctx_t *w, SSL *ssl, const char *client_ip, const unsigned char *key) {
    ...
    if (a && b && !c) {
        ...
        DBG("worker[%d] login request username=%s password=%s ip=%s", w->id, a, b, client_ip);
        ...
        DBG("worker[%d] password hash input prepared salt=%s password=%s combined=%s", w->id, salt, b, combined);

        unsigned char hash_bin[SHA256_DIGEST_LENGTH];
        unsigned int mdlen = 0;
        if (EVP_Digest(combined, strlen(combined), hash_bin, &mdlen, EVP_sha256(), NULL) != 1 ||
        ...
        DBG("worker[%d] comparing password verifier calculated=%s stored=%s", w->id, hash_input, hash);
