#!/bin/bash
#
# check_redfish_ssd_endurance_v2.sh
#
# Purpose:
#   Check HPE iLO Redfish SmartStorage disk drive
...
if ! echo "$SYSTEMS_JSON" | jq empty >/dev/null 2>&1; then
  echo "UNKNOWN - failed to get /redfish/v1/Systems or invalid JSON"
  exit 3
fi
...
if [[ -z "$SYSTEM_URIS" ]]; then
  echo "UNKNOWN - no system URI found from /redfish/v1/Systems"
  exit 3
fi
...
###############################################################################
# Preferred method:
# HPE SmartStorage path.
# This path usually contains SSDEnduranceUtilizationPercentage.
###############################################################################
...
###############################################################################
# Fallback method:
# Standard Redfish Storage path.
# Only used if SmartStorage path found no disk.
###############################################################################
...
if [[ -z "$DRIVE_URIS" ]]; then
  echo "UNKNOWN - no Redfish drive URI found"
  exit 3
fi
...
while read -r DRIVE_URI; do
  ...
  if [[ -z "$ENDURANCE" || "$ENDURANCE" == "null" ]]; then
    UNKNOWN_MSGS+=("${LABEL}: no SSDEnduranceUtilizationPercentage found, media=${MEDIA_TYPE}, model=${MODEL}, health=${HEALTH}, state=${STATE}, uri=${DRIVE_URI}")
    continue
  fi

  if ! [[ "$ENDURANCE" =~ ^[0-9]+$ ]]; then
    UNKNOWN_MSGS+=("${LABEL}: invalid endurance value=${ENDURANCE}, uri=${DRIVE_URI}")
    continue
  fi
...
# If there are valid OK/WARNING/CRITICAL results, do not let missing endurance
# from fallback-style objects turn the whole check into UNKNOWN.
if (( ${#CRIT_MSGS[@]} > 0 )); then
...
# Only print UNKNOWN details when no valid drive endurance data was found.
# This avoids false UNKNOWN noise when SmartStorage already gave valid data.
if (( ${#OK_MSGS[@]} == 0 && ${#WARN_MSGS[@]} == 0 && ${#CRIT_MSGS[@]} == 0 )); then
