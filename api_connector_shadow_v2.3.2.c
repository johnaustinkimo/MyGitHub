 ...
 * RHEL 7 normally ships OpenSSL 1.0.2, while RHEL 8+ use newer APIs.
 * Keep one source tree compatible with both without changing the EPV wire
 * protocol or TLS policy (TLS 1.2 minimum).
 *
 * EPV_FORCE_OPENSSL_102 is test-only: it lets a newer build host compile the
 * legacy branch so CI can catch accidental use of OpenSSL 1.1+ APIs.
 */
#if OPENSSL_VERSION_NUMBER < 0x10100000L || defined(EPV_FORCE_OPENSSL_102)
#define EPV_OPENSSL_LEGACY_102 1
#define EPV_TLS_SERVER_METHOD() SSLv23_server_method()
#define EPV_X509_GET_NOTBEFORE(x) X509_get_notBefore((X509 *)(x))
#define EPV_X509_GET_NOTAFTER(x)  X509_get_notAfter((X509 *)(x))
#else
#define EPV_OPENSSL_LEGACY_102 0
#define EPV_TLS_SERVER_METHOD() TLS_server_method()
#define EPV_X509_GET_NOTBEFORE(x) X509_get0_notBefore((x))
#define EPV_X509_GET_NOTAFTER(x)  X509_get0_notAfter((x))
#endif
...
#define DEFAULT_DB_FILE "/ws/shadow.db"
...
#define EPV_VERSION "2.3.2-shadow"
...
static char *aes_encrypt(const char *plain, const unsigned char *key) {
    ...
    DBG("AES encrypt success plain=%s enc=%s", plain, out);
    free(iv64);
    free(ct64);
    return out;
}
...
static char *aes_decrypt(const char *enc, const unsigned char *key) {
    ...
    buf[pl] = '\0';
    DBG("AES decrypt success enc=%s plain=%s", enc, buf);
...
static char *aes_decrypt_db(const char *enc, const unsigned char *key) {
    ...
    plain[pl] = '\0';
    DBG("AES DB password decrypt success encrypted=%s decrypted=%s", enc, plain);
...
static int validate_snapshot_uri(const char *uri) {
    ...
    static const char *schema_checks[] = {
        "SELECT id, gen01, gen02, gen03, gen04, genconf FROM gen_file LIMIT 0;",
        "SELECT id, epv01, epv02, epv03, epvconf, epveftdate FROM epv_file LIMIT 0;",
        "SELECT id, epa01, epa02, epa03, epa04, epaconf FROM epa_file LIMIT 0;",
        "SELECT id, epb04, epbconf FROM epb_file LIMIT 0;",
        NULL
    };
...
static int publish_snapshot_lkg(int candidate_fd, const struct stat *source_st, int bump_epoch,
    ...
    if (snprintf(tmp_path, sizeof(tmp_path), "%s/.shadow.lkg.%ld.tmp",
...
static int open_worker_db(worker_ctx_t *w) {
    ...
    static const char *SQL_LOGIN =
        "SELECT gen03 AS auth_type "
        "FROM gen_file "
        "WHERE gen01 = ?1 "
        "  AND gen02 = ?2 "
        "  AND ?3 LIKE (REPLACE(gen04, '.0', '%') || '%') "
        "  AND genconf = 'Y' "
        "LIMIT 1";

    static const char *SQL_SECRET =
        "SELECT TRIM(t3.epa03) AS password "
        "FROM epv_file t1 "
        "JOIN gen_file t2 ON t1.epv01 = t2.id "
        "JOIN epa_file t3 ON t1.epv02 = t3.id "
        "JOIN epb_file t4 ON t1.epv03 = t4.id "
        "WHERE t2.gen01 = ?1 "
        "  AND ?2 LIKE (REPLACE(t4.epb04, '.0', '%') || '%') "
        "  AND t3.epa01 = ?3 "
        "  AND t3.epa02 = ?4 "
        "  AND t3.epa04 = ?5 "
        "  AND t1.epvconf = 'Y' "
        ...
        "  AND t3.epaconf = 'Y' "
        "  AND t4.epbconf = 'Y' "
        "  AND t1.epveftdate <= CURRENT_DATE "
        "LIMIT 1";
...
static int db_get_login_info(worker_ctx_t *w, const char *username, const char *password,
                             const char *ip, char *auth, size_t auth_sz) {
    ...
    DBG("worker[%d] login SQL bind username=%s password=%s ip=%s", w->id, username, password, ip);

    if (sqlite3_bind_text(st, 1, username, -1, SQLITE_TRANSIENT) != SQLITE_OK ||
        sqlite3_bind_text(st, 2, password, -1, SQLITE_TRANSIENT) != SQLITE_OK ||
        sqlite3_bind_text(st, 3, ip, -1, SQLITE_TRANSIENT) != SQLITE_OK) {
    ...
    snprintf(auth, auth_sz, "%s", v1 ? (const char *)v1 : "");

    DBG("worker[%d] login SQL result username=%s password=%s ip=%s auth=%s",
        w->id, username, password, ip, auth);
    return 1;
}
...
static char *db_get_secret_password(worker_ctx_t *w, const char *username, const char *ip,
    ...
    DBG("worker[%d] DB password encrypted=%s", w->id, result ? result : "NULL");
    return result;
}
...
static void log_peer_cert(SSL *ssl) {
    ...
    const ASN1_TIME *nb = EPV_X509_GET_NOTBEFORE(peer_cert);
    const ASN1_TIME *na = EPV_X509_GET_NOTAFTER(peer_cert);
    if (nb && na) {
...
static int server_cert_days_remaining(void) {
    /* Read the configured PEM directly instead of depending on
       SSL_CTX_get0_certificate(), whose availability differs across older
       OpenSSL builds.  This path runs only in the periodic health check. */
    ...
    const ASN1_TIME *na = EPV_X509_GET_NOTAFTER(cert);
    if (!na) {
        X509_free(cert);
        return INT_MIN;
    }
    int days = 0;
    int secs = 0;
    int ok = ASN1_TIME_diff(&days, &secs, NULL, na);
    X509_free(cert);
    if (ok != 1) return INT_MIN;
...
static void process_request(worker_ctx_t *w, SSL *ssl, const char *client_ip, const unsigned char *key) {
    ...
    if (a && b && !c) {
        ...
        if (!db_get_login_info(w, a, b, client_ip, auth, sizeof(auth))) {
            DBG("worker[%d] login SQL response null username=%s password=%s ip=%s",
                w->id, a, b, client_ip);
    ...
    if (a && b && c && d) {
        ...
        DBG("worker[%d] DB password decrypted=%s", w->id, pw ? pw : "(null)");
        free(sec);
...
int main(int argc, char **argv) {
    ...
    if (snprintf(g_lkg_path, sizeof(g_lkg_path), "%s/shadow.lkg.db", g_runtime_dir) >= (int)sizeof(g_lkg_path)) {
    ...
    g_ssl_ctx = SSL_CTX_new(EPV_TLS_SERVER_METHOD());
    if (!g_ssl_ctx) ssl_die("SSL_CTX_new failed");
#if !EPV_OPENSSL_LEGACY_102
# if defined(TLS1_2_VERSION)
    if (SSL_CTX_set_min_proto_version(g_ssl_ctx, TLS1_2_VERSION) != 1)
        ssl_die("SSL_CTX_set_min_proto_version TLS1.2 failed");
# endif
#else
    /* OpenSSL 1.0.2 has no SSL_CTX_set_min_proto_version().  Disable every
       protocol older than TLS 1.2 explicitly. */
    (void)SSL_CTX_set_options(g_ssl_ctx, SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3);
# ifdef SSL_OP_NO_TLSv1
    (void)SSL_CTX_set_options(g_ssl_ctx, SSL_OP_NO_TLSv1);
# endif
# ifdef SSL_OP_NO_TLSv1_1
    (void)SSL_CTX_set_options(g_ssl_ctx, SSL_OP_NO_TLSv1_1);
# endif
#endif
    ...
    {
        char stopping[192];
        snprintf(stopping, sizeof(stopping), "STOPPING=1\nSTATUS=EPV API Connector v%s shutting down", EPV_VERSION);
        (void)sd_notify_message(stopping);
    }
