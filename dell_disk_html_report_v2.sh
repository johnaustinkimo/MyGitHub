...
Example:
  chmod +x check_dell_disk_health_v1.sh dell_disk_html_report_v2.sh
  ./dell_disk_html_report_v2.sh -f dell_servers.ini -s ./check_dell_disk_health_v1.sh -o dell_report.html -P 5 -r

Cron example:
  30 8 * * * /opt/ssd/dell_disk_html_report_v2.sh -f /opt/ssd/dell_servers.ini -s /opt/ssd/check_dell_disk_health_v1.sh -o /var/www/html/dell_disk_report.html -P 5 >/tmp/dell_report.log 2>&1
USAGE
}
...
cat <<HTML
<div class="footer">Generated by dell_disk_html_report_v2.sh ${VERSION}. Passwords are read from inventory files but are not written to this report.</div>
</body>
</html>
HTML
} > "$OUT_HTML"
