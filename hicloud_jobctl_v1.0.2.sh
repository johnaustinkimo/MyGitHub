...
[[ -d "$JOB_HOME" ]] || {
    echo "ERROR|JOB_HOME does not exist: $JOB_HOME"
    exit 1
}
...
start_job()
{
    ...
    write_field "$dir" job_home "$JOB_HOME"
    write_field "$dir" job_user_env "$JOB_USER_ENV"
    write_field "$dir" job_logname_env "$JOB_LOGNAME_ENV"

    {
        ...
        echo "[BOOTSTRAP] HOME=$JOB_HOME"
        echo "[BOOTSTRAP] USER=$JOB_USER_ENV"
        echo "[BOOTSTRAP] LOGNAME=$JOB_LOGNAME_ENV"
...
worker()
{
    ...
    # systemd system services do not guarantee shell-style HOME/USER/LOGNAME.
    # The HiCloud CLI resolves ~/.s3s from HOME, so make the execution
    # environment deterministic and equivalent to the successful root shell.
    export HOME="$JOB_HOME"
    export USER="$JOB_USER_ENV"
    export LOGNAME="$JOB_LOGNAME_ENV"

    echo "[WORKER] pid=$$"
    echo "[WORKER] pwd=$(pwd -P)"
    echo "[WORKER] HOME=$HOME"
    echo "[WORKER] USER=$USER"
    echo "[WORKER] LOGNAME=$LOGNAME"
    ...
    if [[ -e "$HOME/.s3s" ]]; then
        echo "[WORKER] s3s_home_exists=yes path=$HOME/.s3s"
        stat -c '[WORKER] s3s_home mode=%a owner=%U:%G type=%F' "$HOME/.s3s" 2>/dev/null || true
    else
        echo "[WORKER] s3s_home_exists=no path=$HOME/.s3s"
    fi

    if [[ -e "$HOME/.s3s/config" ]]; then
        stat -c '[WORKER] s3s_config mode=%a owner=%U:%G type=%F' "$HOME/.s3s/config" 2>/dev/null || true
    else
        echo "[WORKER] s3s_config_exists=no path=$HOME/.s3s/config"
    fi
...
case "${1:-}" in
    ...
    cancel)
        ...
        ;;
    version|--version|-V)
        echo "hicloud_jobctl_v1.0.2.sh v${VERSION}"
        ;;
    *)
        cat <<EOF
