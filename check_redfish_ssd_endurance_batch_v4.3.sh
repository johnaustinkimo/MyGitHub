#!/bin/bash
#
# check_redfish_ssd_endurance_batch_v4.3.sh
#
# Purpose:
...
#
# v4.3 Fix:
#   De-duplicate same physical disk by SerialNumber.
#   Prefer complete Storage/Chassis Drive data over legacy SmartStorage data
#   when both represent the same physical disk.
#
...
#
# Discovery order:
#   1. HPE legacy SmartStorage DiskDrives
#   2. Standard Redfish Storage Drive links
#   3. Standard Redfish Storage Drive collection
#   4. Chassis Drive collection
#
# Safe handling for:
#   "Drives": [ {"@odata.id": "..."} ]
#   "Drives": { "@odata.id": "..." }
...
${found_drives}"
  ...
  ###########################################################################
  # Method 2 and 3: iLO5/iLO6 standard Storage model
  ###########################################################################
  if [[ -n "$system_uris" ]]; then
    local system_uri
...
candidate_score() {
  ...
  if [[ "$name" != "UNKNOWN" && "$name" != "HpeSmartStorageDiskDrive" ]]; then
    score=$((score + 20))
  fi
...
#   3. Skip non-SSD physical drives.
#   4. Build candidates.
#   5. De-duplicate by SerialNumber.
#   6. Keep best candidate per physical SSD.
#   7. Write summary and detail based on de-duplicated result only.
###############################################################################
check_one_server() {
  local server_name="$1"
  local ilo_host="$2"
  ...
  while read -r drive_uri; do
    ...
    if ! echo "$drive_json" | is_json_ok; then
      ...
      name="$(safe_field "$(echo "$drive_json" | jq -r '.Name // .Id // "UnknownDrive"' 2>/dev/null)")"
      model="$(safe_field "$(echo "$drive_json" | jq -r '.Model // "UnknownModel"' 2>/dev/null)")"
      serial="$(safe_field "$(echo "$drive_json" | jq -r '.SerialNumber // .serialNumber // "UnknownSerial"' 2>/dev/null)")"
      media_type="$(safe_field "$(echo "$drive_json" | jq -r '.MediaType // .mediaType // "UnknownMedia"' 2>/dev/null)")"
      health="$(safe_field "$(echo "$drive_json" | jq -r '.Status.Health // "UnknownHealth"' 2>/dev/null)")"
      state="$(safe_field "$(echo "$drive_json" | jq -r '.Status.State // "UnknownState"' 2>/dev/null)")"
      ...
      #######################################################################
      # This script is for SSD endurance.
      # Skip non-SSD drives to avoid HDD / enclosure / virtual drive noise.
      #######################################################################
  ...
  if [[ ! -s "$tmp_final" ]]; then
    ...
    printf '%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s\n' \
      "$server_name" "$ilo_host" "UNKNOWN" "UNKNOWN" "UNKNOWN" "UNKNOWN" "UNKNOWN" \
      "UNKNOWN" "UNKNOWN" "UNKNOWN" "UNKNOWN" "UNKNOWN" \
      "No SSD drive data found after de-duplication" "-" "-" >> "$TMP_DETAIL"

    printf '%s|%s|%s|%s|%s|%s|%s|%s|%s\n' \
      "$server_name" "$ilo_host" "$server_status" 0 0 0 0 1 "No SSD drive data found" >> "$TMP_SUMMARY"
...
Deduplication: Enabled by SerialNumber, best record selected automatically
</div>
...
<div class="summary-cards">
  ...
  <div class="card"><div class="card-title">Total SSD Disks</div><div class="card-value">${TOTAL_DISKS}</div></div>
</div>
...
<div class="footer">
Report generated by check_redfish_ssd_endurance_batch_v4.3.sh
</div>
