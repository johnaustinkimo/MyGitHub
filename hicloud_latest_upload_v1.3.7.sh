...
#
# Usage:
#   hicloud_latest_upload_v1.3.7.sh run
#   hicloud_latest_upload_v1.3.7.sh run testfile-20260917
#   hicloud_latest_upload_v1.3.7.sh check-source
#   hicloud_latest_upload_v1.3.7.sh list
#   hicloud_latest_upload_v1.3.7.sh status
#   hicloud_latest_upload_v1.3.7.sh version
#
# Notes:
...
verify_source_stable()
{
    ...
    if (( count1 <= 0 )); then
        log ERROR "source contains no regular files: $SOURCE_DIR"
        return 1
    fi
...
# statistics are already captured by source_snapshot().
# ============================================================
...
upload_archive()
{
    ...
    chmod 600 "$CURRENT_TOOL_LOG" "$CURRENT_PHASE_FILE"
...
case "$ACTION" in
    ...
    version|--version|-V)
        ...
        echo "hicloud_latest_upload_v1.3.7.sh v${VERSION}"
        ;;
