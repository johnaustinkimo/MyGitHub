#!/bin/bash
# ==============================================================================
# hpe_disk_html_report_v1.sh
# Multi-server HPE iLO disk health HTML report wrapper
#
# Reads one or more servers.ini files in this format:
#   name|ilo_host_or_ip|username|password
#
# Calls existing check_hpe_disk_health.sh for each server, parses CSV output,
# and generates a pretty HTML report with host summary and drive-level details.
# ==============================================================================

set -o pipefail

INPUT_FILE="/ws/ilo_hosts.txt"
#INPUT_FILE="/temp/ilo_hosts.txt"
REPORT_DIR="/var/log/ilo_health_reports"
mkdir -p "$REPORT_DIR"

VERSION="v1"
CHECK_SCRIPT="./check_hpe_disk_health.sh"
OUT_HTML="${REPORT_DIR}/hpe_disk_health_report_$(date '+%Y%m%d_%H%M%S').html"
OUT_PDF="${REPORT_DIR}/hpe_disk_health_report_$(date '+%Y%m%d_%H%M%S').pdf"
WARN_THRESHOLD=70
CRIT_THRESHOLD=90
TIMEOUT=30
PARALLEL=1
SHOW_RAW=false
TMP_KEEP=false
TITLE="HPE iLO Disk Drive Health Report"
...
Required:
  -f FILE     Server inventory file. May be specified multiple times.
              Format: name|ilo_host_or_ip|username|password

Options:
  -s FILE     Path to check_hpe_disk_health.sh (default: ${CHECK_SCRIPT})
  ...
  -t NUM      HTTP timeout seconds per iLO request (default: ${TIMEOUT})
...
Example:
  chmod +x check_hpe_disk_health.sh hpe_disk_html_report_v1.sh
  ./hpe_disk_html_report_v1.sh -f servers.ini -s ./check_hpe_disk_health.sh -o report.html -P 5 -r

Cron example:
  30 8 * * * /opt/ssd/hpe_disk_html_report_v1.sh -f /opt/ssd/servers.ini -s /opt/ssd/check_hpe_disk_health.sh -o /var/www/html/hpe_disk_report.html -P 5 >/tmp/hpe_report.log 2>&1
USAGE
}
...
TMPDIR=$(mktemp -d /tmp/hpe_disk_report.XXXXXX)
...
run_one_host() {
  ...
  # If exit is 0 but parser found issues, prefer parser-derived status.
  if [[ "$unhealthy" -gt 0 || "$ssd_alerts" -gt 0 ]]; then
    status="ACTION REQUIRED"
    class="crit"
  elif [[ "$no_drives" -eq 1 && "$exit_code" -ne 4 ]]; then
    status="NO DRIVES"
    class="warn"
  fi
...
<div class="card">
  ...
  <div class="card-body scroll">
    <table>
      <thead><tr><th>#</th><th>Host name</th><th>iLO IP / Host</th><th>Status</th><th>Total drives</th><th>Unhealthy / failed</th><th>SSD alerts</th><th>Duration</th><th>Exit</th></tr></thead>
      <tbody>
HTML
...
<div class="card">
  <div class="card-header">
    <div>
      ...
      <div class="host-meta">iLO: ${esc_host} | Source: $(printf '%s' "$src_ini" | html_escape) | Duration: ${duration}s</div>
    </div>
...
HTML
    ...
    cat <<HTML
    <p><span class="badge ${class}">${esc_status}</span> No drive rows were parsed. Check connectivity, credentials, iLO Redfish support, or raw output.</p>
HTML
  fi
...
cat <<HTML
<div class="footer">Generated by hpe_disk_html_report_v1.sh ${VERSION}. Passwords are read from inventory files but are not written to this report.</div>
</body>
</html>
HTML
} > "$OUT_HTML"
